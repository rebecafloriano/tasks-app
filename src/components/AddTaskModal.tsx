...
export const AddTaskModal = ({ isOpen, onClose }: AddTaskModalProps) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            ...
            <dialog open className="relative z-10 flex flex-col w-[95%] max-w-md max-h-[90vh] rounded-2xl border border-slate-700 bg-slate-800 p-5 shadow-2xl overflow-hidden">
                <h2 className="text-xl text-slate-100 uppercase font-bold tracking-tight text-center pb-4 border-b border-slate-700/50">Nova Tarefa</h2>
                <form onSubmit={handleSubmit} className="flex flex-col gap-5 mt-4 overflow-y-auto pr-1">
                        ...
                        <div className="flex flex-wrap gap-x-2 gap-y-2.5">
                            {(["Frontend Task", "Backend Task", "UI/UX Task", "FullStack Task"] as Task["category"][]).map((cat) => (
                                <button
                                    ...
                                    className={`uppercase px-3 py-2 text-[11px] font-bold rounded-lg border transition-all cursor-pointer ${category === cat ? "bg-blue-500/20 text-blue-400 border-blue-500" : "bg-slate-900/40 border-slate-700 text-slate-400 active:bg-slate-700"}`}
                                >
                                    {cat}
                                </button>
                            ))}
                        ...
                        <div className="flex gap-2 w-full">
                            {(['HIGH', 'MEDIUM', 'LOW'] as Task["priority"][]).map((prior) => (
                                <button
                                    ...
                                    className={`uppercase text-[11px] font-bold flex-1 text-center py-2.5 rounded-lg border transition-colors cursor-pointer ${priority === prior ? prior === 'HIGH' ? 'text-red-400 bg-red-400/20 border-red-400' : prior === 'MEDIUM' ? 'bg-amber-500/20 text-amber-400 border-amber-500' : 'bg-emerald-500/20 text-emerald-400 border-emerald-500' : "bg-slate-900/40 border-slate-700 text-slate-400 active:bg-slate-700"}`}
                                >
                                    {prior === 'HIGH' && 'Alta'}
                                    {prior === 'MEDIUM' && 'Média'}
                                    {prior === 'LOW' && 'Baixa'}
                    ...
                    <div className="text-sm flex justify-between gap-3 pt-4 border-t border-slate-700/50 mt-2">
                        <button type="button" onClick={onClose} className="font-bold uppercase w-1/2 bg-slate-700 text-slate-300 py-3 rounded-xl text-xs tracking-wider">cancelar</button>
                        <button type="submit" className="font-bold uppercase w-1/2 bg-yellow-500 text-yellow-950 py-3 rounded-xl text-xs tracking-wider shadow-md">Criar Tarefa</button>
                    </div>
                </form>
            </dialog>
        </div>
